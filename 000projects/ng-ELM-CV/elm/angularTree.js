    ...
    f.module("angularTreeview", []).directive("treeModel", function($compile) {
        return {
            ...
            link: function(b, h, c) {
                ...
                d + '.length"></i> <span id="{{node.id}}" data-ng-class="node.selected" data-ng-click="' + a + '.selectNodeLabel(node)">{{node.' + e + '}}</span><div data-ng-hide="node.collapsed" data-tree-id="' + a + '" data-tree-model="node.' + d + '" data-node-id=' + (c.nodeId || "id") + " data-node-label=" + e + " data-node-children=" + d + "></div></li></ul>";
